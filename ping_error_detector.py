...
os.system('chcp 950') # 切換為繁體中文(Big5)
...
response_split = response_split.replace(", "," ").replace(" = "," ").split(" ")
while '' in response_split:
    response_split.remove('')
#print(response_split)

i = 0
while response_split[i] != '已收到' or i == len(response_split):
    i = i+1
socket_receive = int(response_split[i+1]) > 0
j = 0
while response_split[j] != '已遺失' or j == len(response_split):
    j = j+1
...
if not socket_receive and socket_lost:    
    ...
    f_ping.close()
    if ping_Err_count >= 3:                
        print(f"Ping error occurred {ping_Err_count} times!!!")                
        print("System sleep for 1 minute.")
        line_notify_future(f"Ping error occurred {ping_Err_count} times!!!\n"                    f"System sleep for 1 minute") 
        T_module.sleep(60)                
    else:
        print("Ping Error Occurs!!!")
        ping_Err_count = ping_Err_count + 1
        print(f"Ping error times: {ping_Err_count}")
else:            
